...
function sendAjax(url, data, callback, type){
	...
	$.ajax({
		...
		error: function(XMLHttpRequest, textStatus, errorThrown){
			alert('Не удалось выполнить запрос Ajax! Ошибка на сервере.');
		},
	});
}
...
function sendFiles(url, data, callback, type){
	...
	$.ajax({
        ...
        {
            alert('Не удалось выполнить запрос sendFiles! Ошибка на сервере. ' +	errorThrown);
        }
    });
}
...
function popup(content){
	$('body').append('<div class="modal"><div class="modal-dialog"><div class="modal-content">' +
		'<div class="modal-header"><a href="#" class="popup-close close" onclick="return popupClose(this)"><span class="glyphicon glyphicon-remove" style="color: red;"></span></a></div>'+
...
document.addEventListener('DOMContentLoaded', () => {
	if($('#tree').length > 0){
		sendAjax('/admin/stock/tree', {}, function (data) {
			if (typeof data.tree != 'undefined') {
				initTree(data.tree);
				if(typeof stockItemIds !== "undefined"){
					let treeViewObject = $('#tree').data('treeview');
					let nodes = treeViewObject.getUnselected();
					for (let i = 0; i < nodes.length; i++) {
						console.log(nodes[i].value, stockItemIds.indexOf(nodes[i].value));
						if(stockItemIds.indexOf(nodes[i].value) === -1) continue;
						treeViewObject.checkNode(nodes[i].nodeId);
					}
				}
			}
		});
